...
let chcknX = [17, 119, 220, 321, 422];

// Selects random index from chcknX. Math.random returns a random number between 0 inclusive to 1 exclusive. So if were to produced 0.55, that would be multiplied by 5 (in this case), which equals 2.75, then Math.floor would round it down to the nearest integer (unless it's already an integer, which can only be 0), which equals 2, resulting in randomGemX equaling the third index, 220.
let randomChcknX = chcknX[Math.floor(Math.random() * chcknX.length)];

// Y-axis (vertical) values (start from top)
let chcknY = [124, 208, 290, 373];

let randomChcknY = chcknY[Math.floor(Math.random() * chcknY.length)];
...
function playAgain() {
  ...
  score = 0;
  sidebarScore.innerHTML = score;
  // Generates new chckn of random color and random x and y value from arrays
  chckn.x = chcknX[Math.floor(Math.random() * chcknX.length)];
  chckn.y = chcknY[Math.floor(Math.random() * chcknY.length)];
  chckn.sprite = collectibles[Math.floor(Math.random() * 9)];
...
class Player {
  ...
  constructor(x, y, speed) {
    ...
    this.sprite = 'images/boy-farmer.png';
    this.x = x;
    this.y = y;
    this.speed = speed;
...
// Sets enemy's initial location
  constructor(x, y, speed) {
    ...
    this.sprite = 'images/bad-racoon.png';
  }
...
class Chckn {
  constructor(x, y) {
    ...
    this.sprite = collectibles[Math.floor(Math.random() * 9)];
    this.x = randomChcknX;
    this.y = randomChcknY;
    this.imgWidth = 65;
    this.imgHeight = 88;
  ...
  // Draws chckn on screen
  render() {
    ctx.drawImage(Resources.get(this.sprite), this.x, this.y)
  }

  update() {
    // Not sure why this if statement only works when player approaches chckn from below (a higher y value)
    if ( ((Math.abs( (player.imgWidth + player.x) - (this.x + this.imgWidth) ) < 55)) && ((Math.abs( (player.imgHeight + player.y) - (this.y + this.imgHeight) ) < 55)) ) {
      // Generates new chckn of random color and random x and y value from arrays
      this.x = chcknX[Math.floor(Math.random() * chcknX.length)];
      this.y = chcknY[Math.floor(Math.random() * chcknY.length)];
      this.sprite = collectibles[Math.floor(Math.random() * 9)];
      score += 5;
      sidebarScore.innerHTML = score;
    }
...
const collectibles = [
  'images/chckn-one.png',
  'images/chckn-two.png',
  'images/chckn-three.png',
  'images/chckn-eight.png',
  'images/chckn-five.png',
  'images/chckn-nine.png',
  'images/chckn-seven.png',
  'images/chckn-six.png',
  'images/chckn-ten.png',
];

// ENEMY/PLAYER/CHCKN OBJECT INSTANTIATION

let chckn = new Chckn(randomChcknX, randomChcknY);
...
const playerSprites = {
  boy: 'images/boy-farmer.png',
  granPa: 'images/granpa-farmer.png',
  girl: 'images/girl-farmer.png'
};
